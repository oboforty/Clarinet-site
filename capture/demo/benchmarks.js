...
function testWASMmic (size, freqData) {
    ...
    function start_microphone(stream){
      ...
      function process_microphone_buffer(event) {
          ...
          if (startCapture) {
            const N = 8;
            const cutoff = 100;
            capture_and_save(frequencyData, cutoff, N);

            startCapture = false;
          }

          // debug
          //let m = find_notes(frequencyData);
          //$("#frequency")[0].innerText = m;
        }
  ...
  function capture_and_save(freqData, cutoff, N) {
    ...
    freqs = freqs.slice(0, N);

    download_file("frequency.json", JSON.stringify(freqs));
  }